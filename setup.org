...
object1.setCallBack("poorSignal", meditation_callback)
...
{
        {       SynthDef.new(\noise, {
                arg freq = 440, amp = 0.2, vol = 0.2, pha = 0, chron = 1;
                var sig, env, sig2, sig3, gen;

                sig = SinOsc.ar (freq, pha);
                sig2 = LFTri.ar (freq, pha);
                ...
                OSCdef.new(
                        \neurovals,
                        {
                                arg msg, time, addr, port, wildcard;
                                [msg, time, addr, port].postln;
